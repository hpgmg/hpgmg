...
for($x=0;$x<$PF_number;$x++){printf(F "            _mm_prefetch((const char*)(_base+%2d),_MM_HINT_T1);\n",8*$x);}
  ...
  for($x=0;$x<$U;$x+=4){printf(F "                __m256d helmholtz_%02d;\n",$x);}
                        printf(F "                                         _mm_prefetch((const char*)(   phi+ijk+2+$U),_MM_HINT_T0);\n");
for($x=0;$x<$U+8;$x+=4){ # pipelined loads/shuffles of phi
 ...
 if(($x==4)){           printf(F "          const __m256d      phi_%02d = _mm256_insertf128_pd(_mm256_castpd128_pd256(temp_%02d),temp_%02d,1);\n",$x-4,$x-2,$x+0);
 ...
 if(($x>=8)&&($x<$U+4)){printf(F "          const __m256d      phi_%02d = _mm256_insertf128_pd(_mm256_castpd128_pd256(temp_%02d),temp_%02d,1);\n",$x-4,$x-2,$x+0);
                        printf(F "          const __m256d      phi_%02d = _mm256_insertf128_pd(_mm256_castpd128_pd256(temp_%02d),temp_%02d,1);\n",$x-5,$x-3,$x-1);
                        printf(F "          const __m256d      phi_%02d = _mm256_insertf128_pd(_mm256_castpd128_pd256(temp_%02d),temp_%02d,1);\n",$x-3,$x-1,$x+1);}}
  ...
  for($x=0;$x<$U;$x+=4){printf(F "          const __m256d RedBlack_%02d = _mm256_load_pd(RedBlackFP+ij+%2d);\n",$x,$x);}
  for($x=0;$x<$U;$x+=4){printf(F "                             new_%02d = _mm256_sub_pd(phi_%02d,_mm256_mul_pd(RedBlack_%02d,new_%02d));\n",$x,$x,$x,$x);}
                        print  F "          ij+=$U;\n";
  ...
  for($x=0;$x<$U;$x+=4){printf(F "          const __m256d RedBlack_%02d = _mm256_xor_pd(invertMask4,_mm256_load_pd((double*)(RedBlackMask+ij+%2d)));\n",$x,$x);}
                        print  F "          ij+=$U;\n";
  for($x=0;$x<$U;$x+=4){printf(F "                                         _mm256_store_pd(phi+ijk+%2d,_mm256_blendv_pd(phi_%02d,new_%02d,RedBlack_%02d));\n",$x,$x,$x,$x);}
                        print  F "          ijk+=$U;\n";
                        print  F "          #endif\n";
                        #- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                        print  F "        }\n";
