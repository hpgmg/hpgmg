...
subroutine solve(cg,srg,Apply1,Apply2,Relax,Res0,errors,nViters)
  ...
  interface
     subroutine MGSR(srg,cg,fl,sr_ux,sr_rhs,sr_aux,uxC0,auxC0,Apply1,Apply2,Relax,Res0,errors)
  ...
  if (nfcycles .ne. 0) then
     ...
     if (nsr.gt.0) then
        ...
        do lev=-1,-nsr,-1
           ...
           call MGSR(srg,cg,lev,sr_ux,sr_rhs,sr_aux,ux,L2u_f,Apply1,Apply2,Relax,Res0,errors)
        end do
...
!-----------------------------------------------------------------------
! SR: MSRG - last leg of SR, collect functional, evanescent data
!-----------------------------------------------------------------------
subroutine MGSR(srg,cg,lev,sr_ux,sr_rhs,sr_aux,uxC0,auxC0,Apply1,Apply2,Relax,Res0,errors)
  use grid_module
  use pms
  use error_data_module
  use mpistuff,only:mype
...
!!$     if (mype==0)write(6,'(A,I2,A,E12.4,A,E12.4)')'lev=',nsr,') MSRG copy_crs_sr_w_bc_ex |u_dest|=',t2,', error:',tt-t2
  ...
  if (nsmoothsfmg>0) then
     ...
     ! SR2:    pre smoothing
     call Relax(uxF,rhsF,srg(iflv)%p,srg(iflv)%t,nsmoothsfmg)
     if (verbose.gt.2) then
        if (mype==0) print *,'SR:'
