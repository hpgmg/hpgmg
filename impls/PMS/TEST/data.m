...
loglog(N_F256(1:mf256),errors_F_256N(:,1),'r*--'),hold on % error
loglog(N_F256(1:mf256),errors_F_256N(:,2),'go-.'),hold on % residual
...
V(1)=N_F256(1);
V(2)=N_F256(mf256);
V(3)=errors_F_256N(mf256,1)/2;
V(4)=errors_F_256N(1,2)*2;
axis(V);
...
grid
print(gcf,'-djpeg100','converg_Fcycles_256N_hopper')
%
...
loglog(N_F032(1:mf032),errors_F_032N(:,1),'r*--'),hold on
loglog(N_F032(1:mf032),errors_F_032N(:,2),'go-.'),hold on
...
xlabel('N cells X-Y-Z direction');
ylabel('|e_N|_{inf}')
title('Convergence: V-cycles (rtol=1.^{-5}), constant coef. Lapalcian, u=(x^4-x^2) on (0,1)^3, N=256/PE')
grid
print(gcf,'-djpeg100','converg_Vcycles_256N_hopper')
%
...
legend('1 F-cycle w/ V(2,2), N=256/PE, one solve','V(2,2) cycles, N=256/PE, rtol=1.^{-5}',1),
set(gca,'XTick',N_F256(1:mv256))
V=axis;
V(1)=N_F256(1);
V(2)=N_F256(mf256);
V(3)=errors_F_256N(mf256,1)/2;
...
xlabel('N cells X-Y-Z direction');
ylabel('|error|_{\inf}')
title('Error convergence: V-cycles rtol=1.^{-5}, constant coef. Laplacian, u=(x^4-x^2) on (0,1)^3, N=256/PE')
grid
print(gcf,'-djpeg100','converg_errors_hopper')
%
% times
%
figure
...
legend('1 F-cycle w/ V(2,2), N=256/PE, one solve','1 F-cycle w/ V(2,2), N=32/PE, 512 solves','V(2,2) cycles, N=256/PE, rtol=1.^{-5}',2),
xlabel('N PEs (Hopper)');
ylabel('Time')
title('Times: V-cycles (rtol=1.^{-5}), constant coef. Laplacian, u=(x^4-x^2) on (0,1)^3, N=256/PE')
grid
