...
/*
inline void __gemv(double * z, double alpha, double A[((4*__CA_KRYLOV_S)+1)][((4*__CA_KRYLOV_S)+1)], double * x, double beta, double * y, int rows, int cols){ // z[m] = alpha*A[m][n]*x[n]+beta*y[m]   [row][col]
  int r,c;
  for(r=0;r<rows;r++){
    double sum = 0.0;
    for(c=0;c<cols;c++){
...
void TelescopingCABiCGStab(domain_type * domain, int level, int e_id, int R_id, double a, double b, double desired_reduction_in_norm){
  ...
  int mMax=200;
  int m=0,n;
  int maxPower,ghosts=domain->ghosts;
  int i,j,k;
  ...
  residual(domain,level,__rt,e_id,R_id,a,b);                                                     // rt[] = R_id[] - A(e_id)
  ...
  __zero(   aj,4*__CA_KRYLOV_S+1); // zero vectors to avoid garbage when __current_s<__CA_KRYLOV_S
  ...
  while( (m<mMax) && (!BiCGStabFailed) && (!BiCGStabConverged) ){                            // while(not done){
    ...
    // Tp[][] and Tpp[][] must be regenerated as 's' might have changed...
    // initialize Tp[][] and Tpp[][] ...
    for(i=0;i<4*__current_s+1;i++)for(j=0;j<4*__current_s+1;j++) Tp[i][j]=0;                 //
    for(i=0;i<4*__current_s+1;i++)for(j=0;j<4*__current_s+1;j++)Tpp[i][j]=0;                 //
    // monomial basis???
    for(i=              0;i<2*__current_s  ;i++){ Tp[i+1][i]=1;}                             // Fixed (typo in SIAM paper)
    for(i=2*__current_s+1;i<4*__current_s  ;i++){ Tp[i+1][i]=1;}                             // Fixed (typo in SIAM paper)
    for(i=              0;i<2*__current_s-1;i++){Tpp[i+2][i]=1;}
    for(i=2*__current_s+1;i<4*__current_s-1;i++){Tpp[i+2][i]=1;}
  
    #ifdef __DEBUG // print T' and T''
    if(domain->rank==0){
      printf("T' = \n");
      for(i=0;i<4*__current_s+1;i++){printf("| ");
      for(j=0;j<4*__current_s+1;j++){printf("%2.1f ",Tp[i][j]);}printf("|\n");}
      printf("\n");
      printf("T'' = \n");
      for(i=0;i<4*__current_s+1;i++){printf("| ");
      for(j=0;j<4*__current_s+1;j++){printf("%2.1f ",Tpp[i][j]);}printf("|\n");}
      printf("\n");
    } 
    #endif

    // redefine the pointers of PRrt[] as 's' might have changed...
    for(i=0;i<4*__current_s+1;i++){PRrt[              i] = __PRrt+i;}                         // columns of PRrt map to the consecutive spare grids allocated for the bottom solver starting at __PRrt
                                   PRrt[4*__current_s+1] = __rt;                              // last column or PRrt (r tilde) maps to rt
    int *P = PRrt+              0;                                                            // grid_id's of the 2S+1 Matrix Powers of P.  P[i] is the grid_id of A^i(p)
    int *R = PRrt+2*__current_s+1;                                                            // grid_id's of the 2S   Matrix Powers of R.  R[i] is the grid_id of A^i(r)

    // Using the monomial basis, compute 2s+1 matrix powers on p[] and 2s matrix powers on r[] one power at a time (conventional approach applicable to CHOMBO and BoxLib)
    scale_grid(domain,level,P[0],1.0,__p);                                                    // P[0] = A^0p = __p
    for(n=1;n<2*__current_s+1;n++){                                                           // naive way of calculating the monomial basis.
      apply_op(domain,level,P[n],P[n-1],a,b);                                                 // P[n] = A(P[n-1]) = A^(n)p
    }
    scale_grid(domain,level,R[0],1.0,__r);                                                    // R[0] = A^0r = __r
    for(n=1;n<2*__current_s;n++){                                                             // naive way of calculating the monomial basis.
      apply_op(domain,level,R[n],R[n-1],a,b);                                                 // R[n] = A(R[n-1]) = A^(n)r
    } 

    // form G[][] and g[]
    matmul_grids(domain,level,Gg,PRrt,PRrt,4*__current_s+1,4*__current_s+2,1);                // Compute Gg[][] = [P,R]^T * [P,R,rt] (Matmul with grids but only one MPI_AllReduce)
    for(i=0,k=0;i<4*__current_s+1;i++){                                                       // extract G[][] and g[] from Gg[]
    for(j=0    ;j<4*__current_s+1;j++){G[i][j] = Gg[k++];}                                    // first 4*__current_s+1 elements in each row go to G[][].
                                       g[i]    = Gg[k++];                                     // last element in row goes to g[].
    }

    #ifdef __DEBUG // print G[][] and g[]
    if(domain->rank==0){
      printf("G[][] = \n");
      for(i=0;i<4*__current_s+1;i++){//printf("| ");
      for(j=0;j<4*__current_s+1;j++){printf("%10.3e ",G[i][j]);}
                                        printf(";\n");}
      printf("\n");
      printf("g[] = \n");
      for(i=0;i<4*__current_s+1;i++){printf(" %10.3e \n",g[i]);}
      printf("\n");
    } 
    #endif

    for(i=0;i<4*__current_s+1;i++)aj[i]=0.0;aj[              0]=1.0;                          // initialized based on (3.26)
    for(i=0;i<4*__current_s+1;i++)cj[i]=0.0;cj[2*__current_s+1]=1.0;                          // initialized based on (3.26)
    for(i=0;i<4*__current_s+1;i++)ej[i]=0.0;                                                  // initialized based on (3.26)

    #ifdef __DEBUG // print aj[], cj[], and ej[]
    if(domain->rank==0){
      printf("aj[] =           cj[] =           ej[] =           \n");
      for(i=0;i<4*__current_s+1;i++){printf(" %10.3e ,   %10.3e ,   %10.3e ;   \n",aj[i],cj[i],ej[i]);}
      printf("\n");
    }
    #endif

    for(n=0;n<__current_s;n++){                                                               // for(n=0;n<__current_s;n++){
      domain->Krylov_iterations++;                                                            // record number of inner-loop (j) iterations for comparison
      __gemv( Tpaj,   1.0, Tp,   aj,   0.0, Tpaj,4*__current_s+1,4*__current_s+1);            //    T'aj
      __gemv( Tpcj,   1.0, Tp,   cj,   0.0, Tpcj,4*__current_s+1,4*__current_s+1);            //    T'cj
      __gemv(Tppaj,   1.0,Tpp,   aj,   0.0,Tppaj,4*__current_s+1,4*__current_s+1);            //   T''aj
                       g_dot_Tpaj = __dot(g,Tpaj,4*__current_s+1);                            // (g,T'aj)
      ...
      __gemv(temp1,-alpha,  G, Tpaj,   0.0,temp1,4*__current_s+1,4*__current_s+1);            //  temp1[] =       - alpha*GT'aj
      __gemv(temp1,   1.0,  G,   cj,   1.0,temp1,4*__current_s+1,4*__current_s+1);            //  temp1[] =   Gcj - alpha*GT'aj
      __gemv(temp2,-alpha,  G,Tppaj,   0.0,temp2,4*__current_s+1,4*__current_s+1);            //  temp2[] =       − alpha*GT′′aj
      __gemv(temp2,   1.0,  G, Tpcj,   1.0,temp2,4*__current_s+1,4*__current_s+1);            //  temp2[] = GT′cj − alpha*GT′′aj
      __axpy(temp3,   1.0,     Tpcj,-alpha,Tppaj,4*__current_s+1);                            //  temp3[] =  T′cj − alpha*T′′aj
             omega_numerator = __dot(temp3,temp1,4*__current_s+1);                            //  (temp3,temp1) = ( T'cj-alpha*T''aj ,   Gcj-alpha*GT'aj )
           omega_denominator = __dot(temp3,temp2,4*__current_s+1);                            //  (temp3,temp2) = ( T′cj−alpha*T′′aj , GT′cj−alpha*GT′′aj )
      #else                                                                                   // better to change the order of operations Gx-Gy -> G(x-y) ...  (note, G is symmetric)
      __axpy(temp1,   1.0,     Tpcj,-alpha,Tppaj,4*__current_s+1);                            //  temp1[] =  (T'cj - alpha*T''aj)
      __gemv(temp2,   1.0,  G,temp1,   0.0,temp2,4*__current_s+1,4*__current_s+1);            //  temp2[] = G(T'cj - alpha*T''aj)
      __axpy(temp3,   1.0,       cj,-alpha, Tpaj,4*__current_s+1);                            //  temp3[] =     cj - alpha*T'aj
             omega_numerator = __dot(temp3,temp2,4*__current_s+1);                            //  (temp3,temp2) = ( (  cj - alpha*T'aj ) , G(T'cj - alpha*T''aj) )
           omega_denominator = __dot(temp1,temp2,4*__current_s+1);                            //  (temp1,temp2) = ( (T'cj - alpha*T''aj) , G(T'cj - alpha*T''aj) )
      #endif                                                                                  // 
      ...
      __axpy(   ej,1.0,ej,       alpha,   aj,4*__current_s+1);                                // ej[] = ej[] + alpha*aj[]    

      // calculate the norm of Saad's vector 's' to check intra s-step convergence...
      __axpy(temp1,   1.0,       cj,-alpha, Tpaj,4*__current_s+1);                            //  temp1[] =   cj - alpha*T'aj
      __gemv(temp2,   1.0,  G,temp1,   0.0,temp2,4*__current_s+1,4*__current_s+1);            //  temp2[] = G(cj - alpha*T'aj)
                                L2_norm_of_s = __dot(temp1,temp2,4*__current_s+1);            //  (temp1,temp2) = ( (cj - alpha*T'aj) , G(cj - alpha*T'aj) )  == square of L2 norm of s in exact arithmetic
      if(L2_norm_of_s<0)L2_norm_of_s=0;else L2_norm_of_s=sqrt(L2_norm_of_s);                  // finite precision can lead to the norm^2 being < 0 (Jim Demmel says flush to zero)
      #ifdef __VERBOSE                                                                        //
      ...
      __axpy(   ej,1.0,ej,       omega,   cj,4*__current_s+1);                                // ej[] = ej[] + alpha*aj[] + omega*cj[]
      __axpy(   ej,1.0,ej,-omega*alpha, Tpaj,4*__current_s+1);                                // ej[] = ej[] + alpha*aj[] + omega*cj[] - omega*alpha*T'aj[]
      __axpy(   cj,1.0,cj,      -omega, Tpcj,4*__current_s+1);                                // cj[] = cj[] - omega*T'cj[]
      __axpy(   cj,1.0,cj,      -alpha, Tpaj,4*__current_s+1);                                // cj[] = cj[] - omega*T'cj[] - alpha*T'aj[]
      __axpy(   cj,1.0,cj, omega*alpha,Tppaj,4*__current_s+1);                                // cj[] = cj[] - omega*T'cj[] - alpha*T'aj[] + omega*alpha*T''aj[]


      // calculate the norm of the incremental residual (Saad's vector 'r') to check intra s-step convergence...
      __gemv(temp1,   1.0,  G,   cj,   0.0,temp1,4*__current_s+1,4*__current_s+1);            // temp1[] = Gcj
                                     cj_dot_Gcj = __dot(cj,temp1,4*__current_s+1);            // sqrt( (cj,Gcj) ) == L2 norm of the intermediate residual in exact arithmetic
      L2_norm_of_residual = 0.0;if(cj_dot_Gcj>0)L2_norm_of_residual=sqrt(cj_dot_Gcj);         // However, finite precision can lead to the norm^2 being < 0 (Jim Demmel says flush to zero)
      #ifdef __VERBOSE 
      ...
      delta_next = __dot( g,cj,4*__current_s+1);                                              // (g,cj)
      #ifdef __VERBOSE                                                                        //
      if(domain->rank==0){                                                                    //
      ...
      __axpy(   aj,1.0,cj,        beta,   aj,4*__current_s+1);                                // aj[] = cj[] + beta*aj[]
      __axpy(   aj,1.0,aj, -omega*beta, Tpaj,4*__current_s+1);                                // aj[] = cj[] + beta*aj[] - omega*beta*T'aj
    ...
    // update iterates...
//  __axpy(temp1,1.0,cj,-alpha,Tpaj,4*__current_s+1);                                         // temp1[] =      cj-alpha*T'*aj
//                                 add_grids(domain,level, __s,0.0, __s,temp1[0],PRrt[0]);    //    s[] = [P,R](cj-alpha*T'*aj) = [P,R]temp1[]
//  for(i=1;i<4*__current_s+1;i++){add_grids(domain,level, __s,1.0, __s,temp1[i],PRrt[i]);}   //          ...
    for(i=0;i<4*__current_s+1;i++){add_grids(domain,level,e_id,1.0,e_id,ej[i],PRrt[i]);}      // e_id[] = [P,R]ej + e_id[]
    ...
    for(i=1;i<4*__current_s+1;i++){add_grids(domain,level, __p,1.0, __p,aj[i],PRrt[i]);}      //          ...
                                   add_grids(domain,level, __r,0.0, __r,cj[0],PRrt[0]);       //    r[] = [P,R]cj
    for(i=1;i<4*__current_s+1;i++){add_grids(domain,level, __r,1.0, __r,cj[i],PRrt[i]);}      //          ...
    }
                              m+=__current_s;                                                 //   m+=__current_s;

    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    //One can calculate the norm of the true residual after s-steps.  However, this is superfluous if you are calculating (cj,Gcj) and expensive as it adds another MPI_AllReduce()
    //residual(domain,level,__temp,e_id,R_id,a,b);                                                 // true residual
    //double norm_of_residual = norm(domain,level,__temp);                                         // norm of true residual
    //#ifdef __VERBOSE                                                                             //
    //if(domain->rank==0)printf("m=%8d, norm   =%0.20f\n",m,norm_of_residual);                     //
    //#endif                                                                                       //
    //if(norm_of_residual < desired_reduction_in_norm*norm_of_rt){BiCGStabConverged=1;break;}      // convergence ?
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    __current_s = 2*__current_s;
    if(__current_s>__CA_KRYLOV_S)__current_s=__CA_KRYLOV_S;
  }                                                                                              // } // outer m loop
...
void CABiCGStab(domain_type * domain, int level, int e_id, int R_id, double a, double b, double desired_reduction_in_norm){
  ...
  double      g[4*__CA_KRYLOV_S+1];                                                             // extracted from last [4*__CA_KRYLOV_S+1] column of Gg[][].
  double   Gg[(4*__CA_KRYLOV_S+1)*(4*__CA_KRYLOV_S+2)];                                        // buffer to hold the Gram-like matrix produced by matmul().  indexed as [row*(4*__CA_KRYLOV_S+2) + col]
  //double *Gg = (double*)malloc((4*__CA_KRYLOV_S+1)*(4*__CA_KRYLOV_S+2)*sizeof(double));
  int      PRrt[4*__CA_KRYLOV_S+2];                                                             // grid_id's of the concatenation of the 2S+1 matrix powers of P, 2S matrix powers of R, and rt
  int *P = PRrt+                 0;                                                              // grid_id's of the 2S+1 Matrix Powers of P.  P[i] is the grid_id of A^i(p)
  ...
  // monomial basis???
  for(i=                 0;i<2*__CA_KRYLOV_S  ;i++){ Tp[i+1][i]=1;}                             // Fixed (typo in SIAM paper)
  for(i=2*__CA_KRYLOV_S+1;i<4*__CA_KRYLOV_S  ;i++){ Tp[i+1][i]=1;}                             // Fixed (typo in SIAM paper)
  ...
  if(domain->rank==0){
    printf("T' = \n");
    for(i=0;i<4*__CA_KRYLOV_S+1;i++){printf("| ");
    for(j=0;j<4*__CA_KRYLOV_S+1;j++){printf("%2.1f ",Tp[i][j]);}printf("|\n");}
    printf("\n");
    printf("T'' = \n");
    for(i=0;i<4*__CA_KRYLOV_S+1;i++){printf("| ");
    for(j=0;j<4*__CA_KRYLOV_S+1;j++){printf("%2.1f ",Tpp[i][j]);}printf("|\n");}
    printf("\n");
  } 
  #endif

  for(i=0;i<4*__CA_KRYLOV_S+1;i++){PRrt[                 i] = __PRrt+i;}                        // columns of PRrt map to the consecutive spare grids allocated for the bottom solver starting at __PRrt
  ...
  while( (m<mMax) && (!BiCGStabFailed) && (!BiCGStabConverged) ){                                // while(not done){
    ...
    // form G[][] and g[]
    matmul_grids(domain,level,Gg,PRrt,PRrt,4*__CA_KRYLOV_S+1,4*__CA_KRYLOV_S+2,1);             // Compute Gg[][] = [P,R]^T * [P,R,rt] (Matmul with grids but only one MPI_AllReduce)
    ...
    if(domain->rank==0){
      printf("G[][] = \n");
      for(i=0;i<4*__CA_KRYLOV_S+1;i++){//printf("| ");
      for(j=0;j<4*__CA_KRYLOV_S+1;j++){printf("%10.3e ",G[i][j]);}
                                        printf(";\n");}
      printf("\n");
      printf("g[] = \n");
      for(i=0;i<4*__CA_KRYLOV_S+1;i++){printf(" %10.3e \n",g[i]);}
      printf("\n");
    } 
    #endif
    ...
    if(domain->rank==0){
      printf("aj[] =           cj[] =           ej[] =           \n");
      for(i=0;i<4*__CA_KRYLOV_S+1;i++){printf(" %10.3e ,   %10.3e ,   %10.3e ;   \n",aj[i],cj[i],ej[i]);}
      printf("\n");
    }
    #endif
...
//  __axpy(temp1,1.0,cj,-alpha,Tpaj,4*__CA_KRYLOV_S+1);                                         // temp1[] =      cj-alpha*T'*aj
//                                    add_grids(domain,level, __s,0.0, __s,temp1[0],PRrt[0]);    //    s[] = [P,R](cj-alpha*T'*aj) = [P,R]temp1[]
//  for(i=1;i<4*__CA_KRYLOV_S+1;i++){add_grids(domain,level, __s,1.0, __s,temp1[i],PRrt[i]);}   //          ...
...
void CACG(domain_type * domain, int level, int e_id, int R_id, double a, double b, double desired_reduction_in_norm){
  ...
  // initialize Tp[][] ...
  for(i=0;i<2*__CA_KRYLOV_S+1;i++)for(j=0;j<2*__CA_KRYLOV_S+1;j++) Tp[i][j]=0;                 //
  // monomial basis???
  for(i=               0;i<  __CA_KRYLOV_S  ;i++){ Tp[i+1][i]=1;}                               //
  ...
  while( (m<mMax) && (!CGFailed) && (!CGConverged) ){                                            // while(not done){
    ...
    // Using the monomial basis, compute s+1 matrix powers on p[] and s matrix powers on r[] one power at a time (conventional approach applicable to CHOMBO and BoxLib)
    scale_grid(domain,level,P[0],1.0,__p);                                                       // P[0] = A^0p = __p
    ...
    for(n=0;n<__CA_KRYLOV_S;n++){                                                               // for(n=0;n<__CA_KRYLOV_S;n++){
      ...
      L2_norm_of_residual = 0.0;if(cj_dot_Gcj_new>0)L2_norm_of_residual=sqrt(cj_dot_Gcj_new);    // However, finite precision can lead to the norm^2 being < 0 (Jim Demmel says flush to zero)
    ...
    //residual(domain,level,__temp,e_id,R_id,a,b);                                                 // true residual
    //double norm_of_residual = norm(domain,level,__temp);                                         // norm of true residual
    //if(norm_of_residual < desired_reduction_in_norm*norm_of_r0){CGConverged=1;break;}      // convergence ?
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  }                                                                                              // } // outer m loop
...
void IterativeSolver(domain_type * domain, int level, int e_id, int R_id, double a, double b, double desired_reduction_in_norm){ 
  ...
  #warning Defaulting to simple bottom solver with fixed number of smooths...
    ...
    for(s=0;s<numSmoothsBottom;s+=numSmooths)smooth(domain,level,e_id,R_id,a,b);
  #endif
  if(a==0){ // i.e. poisson with periodic boundaries doesn't have unique solutions
  double average_value_of_e = mean(domain,level,e_id);
  shift_grid(domain,level,e_id,e_id,-average_value_of_e);
  }
}
