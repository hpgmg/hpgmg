...
//int log2_coarse_dim = 0; // i.e. coarsen to 1^3
  ...
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  #ifdef __USE_MEHRSTELLEN
  #warning using a Merhstellen correction to the RHS
  if(MPI_Rank==0){printf("applying the Merhstellen Correction... f -> f + (h^2/12)Af to the RHS...");fflush(stdout);}
   apply_op(&domain,0,__temp,__f,a,b);			// __temp = Af
  add_grids(&domain,0,__f,1.0,__f,h0*h0/12.0,__temp);	// __f = f + (h^2/12)*__temp
  if(MPI_Rank==0){printf("done\n");fflush(stdout);}
  #endif
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  int s,sMax=2;
  ...
  #endif
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  // Make initial an guess for u(=0)... Solve Lu=f to absolute precision of 1e-10... print the benchmarking timing results...
  MGResetTimers(&domain);for(s=0;s<sMax;s++){zero_grid(&domain ,0,__u); MGSolve(&domain ,__u,__f,a,b,1e-15);}print_timing(&domain );
//MGResetTimers(&domain);for(s=0;s<sMax;s++){zero_grid(&domain ,0,__u);FMGSolve(&domain ,__u,__f,a,b,1e-15);}print_timing(&domain );
  //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
  // calculate error...
  double h3 = h0*h0*h0;
