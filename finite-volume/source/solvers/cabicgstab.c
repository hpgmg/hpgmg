...
#define __USE_TELESCOPING_CA_KRYLOV
//------------------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------------------
#ifdef __USE_TELESCOPING_CA_KRYLOV
void CABiCGStab(level_type * level, int e_id, int R_id, double a, double b, double desired_reduction_in_norm){
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  residual(level,__rt,e_id,R_id,a,b);                                                           // rt[] = R_id[] - A(e_id)... note, if DPC, then rt = R-AD^-1De
  scale_grid(level,__r,1.0,__rt);                                                               // r[] = rt[]
  scale_grid(level,__p,1.0,__rt);                                                               // p[] = rt[]
  double norm_of_rt = norm(level,__rt);                                                         // the norm of the initial residual...
  #ifdef __VERBOSE
  if(level->my_rank==0)printf("m=%8d, norm   =%0.20f\n",m,norm_of_rt);
  #endif
  if(norm_of_rt == 0.0){BiCGStabConverged=1;}                                                   // entered BiCGStab with exact solution
  delta = dot(level,__r,__rt);                                                                  // delta = dot(r,rt)
  ...
  while( (m<mMax) && (!BiCGStabFailed) && (!BiCGStabConverged) ){                               // while(not done){
    ...
    matmul_grids(level,Gg,PRrt,PRrt,4*__ca_krylov_s+1,4*__ca_krylov_s+2,1);
    ...
    // update iterates...
    for(i=0;i<4*__ca_krylov_s+1;i++){add_grids(level,e_id,1.0,e_id,ej[i],PRrt[i]);}             // e_id[] = [P,R]ej + e_id[]
    if(!BiCGStabFailed && !BiCGStabConverged){                                                  // if we're done, then there is no point in updating these
                                     add_grids(level, __p,0.0, __p,aj[0],PRrt[0]);              //    p[] = [P,R]aj
    for(i=1;i<4*__ca_krylov_s+1;i++){add_grids(level, __p,1.0, __p,aj[i],PRrt[i]);}             //          ...
                                     add_grids(level, __r,0.0, __r,cj[0],PRrt[0]);              //    r[] = [P,R]cj
    for(i=1;i<4*__ca_krylov_s+1;i++){add_grids(level, __r,1.0, __r,cj[i],PRrt[i]);}             //          ...
    }                                                                                           //
...
//------------------------------------------------------------------------------------------------------------------------------
#else // __USE_TELESCOPING_CA_KRYLOV =0
void CABiCGStab(level_type * level, int e_id, int R_id, double a, double b, double desired_reduction_in_norm){
  // based on Erin Carson/Jim Demmel/Nick Knight's s-Step BiCGStab Algorithm 3.4
  int  __rt   = __Components+0;
  int  __r    = __Components+1;
