...
\title{HPGMG: A Benchmark for Ranking High Performance Computing Systems}

\begin{abstract}
This document provides an introduction to a benchmark HPGMG for ranking large scale general purpose computers for use on the Top500 list ~\cite{top500}.
We provide a rational for the need for a replacement for the current metric HPL, some background of the Top500 list and the challenges of developing such a metric; we discuss our design philosophy and methodology, and an overview of the specification of the benchmark.
More details on the specification on the repository Wiki (https://bitbucket.org/hpgmg/hpgmg/wiki) and the benchmark code itself can be found in the repository (https://bitbucket.org/hpgmg/hpgmg).
\end{abstract}
\end{frontmatter}
...
In fact, the SCALES report of ~\cite{SCALES} observed that the history of performance improvements in algorithms
...
%Table \ref{tab:ai} shows a list of equations and discretizations with their modeled arithmetic intensity (AI) and measured flop rates.
%\begin{table}[h!]
%\begin{center}
%\begin{tabular}{|c|c|c|c|}
%\multicolumn{3}{c}{}{}{} \\
%\hline
%Equation/discretization         & $AI_p$ & $AI_c$ & flop rate \% \\
%\hline
      %\midrule
%       Q1/FE            &  & &    \\
%       Q2/FE            &  & &  \\
...
%\hline  
%\end{tabular}
%\caption{Arithmetic intensity for discretized Laplacian (FE: finite element; FV: finite volume): perfect cache $AI_p$, finite cache $AI_c$, flop rates as \% of theoretical peak }
%\label{tab:ai}
%\end{center}
%\end{table}
%This data is used in our selection of an equation and discretization that possesses an AI that is shown to be desirable in a metric.
%Thus, we can tune the AI to match applications, encourage machines and hence future application optimizations to use higher AI, and to satisfy other criteria for ranking machines.
...
HPGMG is managed with a core group of developers, an executive committee, and mailing lists (users@hpgmg.org) and periodic public meetings and teleconferences.
A web site at hpgmg.org will contain information on meeting, teleconferences, etc.
The codes and documentation are located in a publicly readable {\it Git} repository at bitbucket.org \cite{HPGMG}.
Users can develop submission codes on branches in the bitbucket.org repository with the stipulation that these branches must be made public or merged with the master branch when the top500 list is published.
The public and post pull requests to modify the the HPGMG codes and documentation.

HPGMG is designed, developed, and maintained by a core group of developers:
\begin{description}
\item[Secretary] {\it Mark F. Adams} Lawrence Berkeley National Laboratory
\item[Developer]  {\it Samuel Williams} Lawrence Berkeley National Laboratory
\item[Developer]  {\it Jed Brown} Argonne National Laboratory
\item[Consultant] {\it Brian Van Straalen} Lawrence Berkeley National Laboratory
\end{description}

An executive committee composed of industry, academic and DOE personnel will approve allocation of resources, approve developer personnel, and provide advice on the content and structure of the benchmark.
%\begin{itemize}
%\item  {\it Erich Strohmaier}
%\item {\it Barry Smith}
%\item 
%\item 
%\end{itemize}
